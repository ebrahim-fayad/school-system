    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form action="{{ route('admin.sections.update', $list_Sections->id) }}" method="POST">
                    {{ method_field('patch') }}
                    {{ csrf_field() }}
                    <div class="row">
                        <div class="col">
                        ...
                        <select name="Grade_id" class="custom-select" onclick="console.log($(this).val())">
                            ...
                            @foreach ($Grades as $list_Grade)
                                <option value="{{ $list_Grade->id }}"
                                    {{ $Grade->id == $list_Grade->id ? 'selected' : '' }}>
                                    {{ $list_Grade->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="col">
                        <div class="form-check">
                            ...
                            @if ($list_Sections->Status === 1)
                                <input type="checkbox" checked class="form-check-input" name="Status" value="1"
                                    id="exampleCheck1">
                            @else
                        ...
                        <div class="col">
                            ...
                            <select multiple name="teachers[]" class="form-control" id="exampleFormControlSelect2">
                                @foreach ($Teachers as $teacher)
                                    <option value="{{ $teacher->id }}"
                                       {{ $list_Sections->teachers->contains($teacher->id) ? 'selected' : ''  }}>
                                        {{ $teacher->Name }}
                                    </option>
                                @endforeach
                            </select>
